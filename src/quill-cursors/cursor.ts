...
export default class Cursor {
  ...
  private _selectionBlock(selection: ClientRect, container: ClientRect, scale: number): HTMLElement {
    ...
    element.style.top = `${(selection.top - container.top) / scale}px`;
    element.style.left = `${(selection.left - container.left) / scale}px`;
    element.style.width = `${(selection.width) / scale}px`;
    element.style.height = `${(selection.height) / scale}px`;
